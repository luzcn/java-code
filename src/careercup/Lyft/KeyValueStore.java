...
public class KeyValueStore {
   ...
   *
   * the get-then-set pattern on global version number may introduce race condition use synchronized
   * keyword to make this function thread safe
   *
  ...
  public synchronized int put(String key, int value) {
    ...
    mapKeyVersionedValue.computeIfAbsent(key, k -> new TreeSet<>())
        .add(new VersionedValue(value, currentVersion));
  ...
  /**
   * get the value of given key with version, if the key has no value with the given number, return
   * the closest version value
   *

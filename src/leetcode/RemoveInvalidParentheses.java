...
public class RemoveInvalidParentheses {
    ...
    private void dfs(String s, int index, String current, int open) {
        ...
        if (!this.result.isEmpty() && (current.length() + s.length() - index < this.result.get(this.result.size() - 1)
                .length())) {
            return;
        }

        if (index == s.length()) {
            if (open == 0 && (this.result.isEmpty() || current.length() == this.result.get(this.result.size() - 1)
                    .length())) {
    ...
    private void bfs(String s) {
        ...
        while (!queue.isEmpty()) {
            ...
            if (isValid(current)) {
                ...
                if (this.result.isEmpty() || current.length() == this.result.get(this.result.size() - 1).length()) {
                    result.add(current);
                }
                continue;
            }
